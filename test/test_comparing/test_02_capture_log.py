    ...
    async def test_execute_plsql_and_sql(self):
        try:
            ...
            for i, test_case in enumerate(test_cases, 1):
                ...
                # * 1. SQL 실행 (데이터 삽입)
                sql_result = await execute_sql(test_case["sql"])
                self.assertTrue(sql_result, f"테스트 케이스 {i}: SQL 실행 실패")
                logging.info(f"테스트 케이스 {i}: SQL 실행 완료")
                ...
                # * 2. PL/SQL 프로시저 실행
                plsql_result = await execute_plsql('TPX_MAIN', test_case["params"])
                self.assertTrue(plsql_result, f"테스트 케이스 {i}: PL/SQL 실행 실패")
                logging.info(f"테스트 케이스 {i}: PL/SQL 실행 완료")
