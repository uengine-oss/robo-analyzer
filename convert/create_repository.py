...
# 반환값: 없음
async def merge_jpa_query_method(repository_code_list, repository_pascal_name, repository_camel_name, lower_name):
    ...
    try:        
        ...
        repository_interface = f"""package com.example.{lower_name}.repository;
import java.util.List;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.data.rest.core.annotation.RepositoryRestResource;
import com.example.{lower_name}.entity.{repository_pascal_name};
...
{merged_methods}
    ...
    """
        logging.info("리포지토리 인터페이스 생성 시작")

        # * 완성된 리포지토리 인터페이스를 저장할 경로를 설정합니다
        base_directory = os.getenv('DOCKER_COMPOSE_CONTEXT', 'convert')
        entity_directory = os.path.join(base_directory, 'converting_result', f'{lower_name}', 'src', 'main', 'java', 'com', 'example', f'{lower_name}', 'repository')
...
#   - node_id : 현재 노드 id
# 반환값: 
#   - variable_node_list : 변수 이름으로 구성된 리스트.
...
#      - lower_name : 소문자 프로젝트 이름
# 반환값: JPA 메서드 리스트.
async def check_tokens_and_process(table_link_node, connection, lower_name):
        ...
        for item in table_link_node:
            ...
            if total_tokens + item_tokens + variable_tokens >= 1500:
                ...
                if not table_link_node_chunk:     
                    ...
                    variable_node_list, variable_tokens = await process_variable_nodes(item['startLine'], connection) 
                    variable_nodes_context[f'variables_{item["startLine"]}'] = variable_node_list
...
#      - lower_name : 소문자 프로젝트 이름
# 반환값: JPA 메서드 리스트
async def start_repository_processing(table_node_list, lower_name):
    
    try:
        connection = Neo4jConnection()
        filtered_table_info_list = []
        ...
        for node in table_node_list:
            ...
            filtered_table_info_list.extend(filtered_table_info)
