...
async def analysis(data, file_content, service_skeleton, jpa_method_list, procedure_variable):
    ...
    async def process_analysis_results():
        ...
        try:
            # * llm에게 데이터를 전달하여, Service 클래스 생성을 위한 결과를 받아옵니다.
            analysis_result = convert_code(clean_code, service_code, variable_list, jpa_query_methods, procedure_variables, fileName)
            LLM_count += 1


            # * llm의 결과 각각의 데이터를 추출하고, 필요한 변수를 초기화합니다 
            service_code = analysis_result['code']


            # * 서비스 클래스 생성을 위한 경로를 설정합니다.
            base_directory = os.getenv('DOCKER_COMPOSE_CONTEXT', 'convert')
            service_directory = os.path.join(base_directory, 'converting_result', f'{fileName}', 'src', 'main', 'java', 'com', 'example', f'{fileName}','service')
            os.makedirs(service_directory, exist_ok=True) 
            service_file_path = os.path.join(service_directory, f"{analysis_result['name']}.java")
            ...
            logging.error("An error occurred during analysis results processing(converting)")
            raise
...
# 반환값: 없음 
async def start_service_processing(service_skeleton, jpa_method_list, procedure_variable, sp_fileName):
    
    global fileName
    fileName = sp_fileName

    try:
        # * 파일의 위치를 기반으로 분석할 스토어드 프로시저 파일 경로를 설정합니다.
        base_dir = os.path.dirname(__file__)  
        analysis_file_path = os.path.abspath(os.path.join(base_dir, '..', 'cypher', 'analysis', f'{fileName}.json'))
        sql_file_path = os.path.abspath(os.path.join(base_dir, '..', 'cypher', 'sql', f'{fileName}.txt'))
