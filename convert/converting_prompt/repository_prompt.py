...
2. Used Variable는 현재 구문에서 사용된 변수 목록으로, 해당 데이터를 참고해서 JPA Query Methods의 매개변수로 식별하세요.
...
3. 쿼리가 매우 복잡한 경우에만 @Query 어노테이션을 사용하고, 되도록이면 네이밍 규칙을 이용해서 JPA Query Methods를 생성하세요
...
** 중요 ** : Repository Interface의 전체 틀이 아닌, 오로직 JPA Query Method만 'code'의 결과에 담아서 반환하세요.
...
{{
    ...
    "code": "Type JPAQueryMethod(@Param("Type Column") Type Field, ...)",
    "methodList" : {{
