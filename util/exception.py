...
class GenerationTemplateError(ConvertingError):
...
class TraversalError(UnderstandingError, ConvertingError):
    def __init__(self, message="데이터 순회 및 탐색 과정에서 오류가 발생했습니다."):
        super().__init__(message)
...
class NodeDataProcessingError(UnderstandingError, ConvertingError):
    def __init__(self, message="노드 데이터를 처리하는 도중 오류가 발생했습니다."):
        super().__init__(message)
